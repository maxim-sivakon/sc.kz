...
//				'width' => 400,
	...
	private function getDocumentMemberCollection(array $requestFilter): MemberCollection
	{
		...
		if (isset($requestFilter['MEMBER_STATUS']) && is_array($requestFilter['MEMBER_STATUS']))
		{
			if (
				...
				&& $document->status !== \Bitrix\Sign\Type\DocumentStatus::STOPPED
			)
			{
			...
			{
				$signed = [...$signed, MemberStatus::STOPPED, MemberStatus::WAIT, MemberStatus::STOPPABLE_READY];
			}
		...
		$this->arResult['COUNTER_ITEMS'] = $this->getCounterItems($memberCollection, $document);

		$sortCallback = Closure::fromCallable([$this, 'sortMembersForDocumentType']);
		$items = $memberCollection->sort($sortCallback)->toArray();
		$limit = $this->getLimitForQuery();
		$page = $this->getNavigation()->getCurrentPage() - 1;
		if ($memberCollection->count() > $limit)
		{
			$chunkedItems = array_chunk($items, $limit);
			$items = $chunkedItems[$page];
		}
		...
		$resultCollection->setQueryTotal($memberCollection->getQueryTotal());

		return $resultCollection;
	}

	public static function sortMembersForDocumentType(Item\Member $memberA, Item\Member $memberB): int
	{
		$roleRelevance = [
			Role::REVIEWER => 100,
			Role::EDITOR => 200,
		...
		if ($memberA->party < $memberB->party)
		{
			return -1;
		}
		elseif ($memberA->party > $memberB->party)
		{
			return 1;
		}

		if ($roleRelevance[$memberA->role] < $roleRelevance[$memberB->role])
		{
			return -1;
		}
		elseif ($roleRelevance[$memberA->role] > $roleRelevance[$memberB->role])
		{
			return 1;
		}

		if ($statusRelevance[$memberA->status] < $statusRelevance[$memberB->status])
		{
			return -1;
		}
		elseif ($statusRelevance[$memberA->status] > $statusRelevance[$memberB->status])
		{
			return 1;
		}
	...
	private function getFilterForQuery(array $requestFilter): ConditionTree
	{
		...
		{
			if (in_array(MemberStatus::READY, $requestFilter['MEMBER_STATUS'], true))
			{
				$requestFilter['MEMBER_STATUS'][] = MemberStatus::STOPPABLE_READY;
			}
	...
	private function getDocumentsDataForGrid(MemberCollection $memberCollection, Item\DocumentCollection $documentCollection): array
	{
		...
		foreach ($memberCollection as $member)
		{
			...
			{
				$memberData['ROLE'] = $this->getRoleInfo($member->role);
			}
			...
			{
				$memberData['MEMBER_STATUS'] = self::calculateStatus($member, $document, $memberCollection);
			}
	...
	private function getAvailableFilterItems(): array
	{
		return [
			...
			'role' => [
				...
				'items' => array_combine(
					Role::getAll(),
					array_map(fn($role): string => $this->getRoleInfo($role), Role::getAll())
				),
			],
			'memberStatus' => [
				'id' => 'MEMBER_STATUS',
				...
				'items' => $this->getMemberAllStatuses(),
			],
			'representative' => [
				'id' => 'REPRESENTATIVE',
	...
	private function getCounterItems(MemberCollection $memberCollection, Item\Document $document): array
	{
		$refusedMemberCount = $this->calculateRefusedMembers($memberCollection);
		$readyMemberCount = $this->calculateReadyMembers($memberCollection, $document);
		$successMemberCount = $this->calculateSuccessMembers($memberCollection);
		return [
			[
			...
			[
				...
				'color' => $readyMemberCount === 0 ? 'THEME' : 'WARNING',
				'isRestricted' => false,
			],
			[
	...
	private function calculateRefusedMembers(MemberCollection $members): int
	{
		$rule = static fn (Item\Member $member) => (
			$member->status === MemberStatus::REFUSED
			&& $member->role === Role::SIGNER
		);

		return $members->filter($rule)->count();
	}

	private function calculateReadyMembers(MemberCollection $members, Item\Document $document): int
	{
		if ($document->status === \Bitrix\Sign\Type\DocumentStatus::STOPPED)
		{
			$rule = static fn (Item\Member $member) => (
	...
	private function calculateSuccessMembers(MemberCollection $members): int
	{
		$rule = static fn (Item\Member $member) => (
			$member->status === MemberStatus::DONE
			&& $member->role === Role::SIGNER
	...
	private function getRoleInfo(string $role): string
	{
		return match ($role) {
	...
	private function getMemberAllStatuses(): array
	{
		return [
	...
	private function prepareBannerParams(): void
	{
		...
		if ($this->type === self::SAFE_TYPE )
		{
			...
			$this->arResult['BANNER_TEXT'] = Loc::getMessage('SIGN_DOCUMENT_GRID_BANNER_SAFE_TEXT');
		}
		elseif ($this->type === self::PERSONAL_TYPE)
		{
			...
			$this->arResult['BANNER_TEXT'] = Loc::getMessage('SIGN_DOCUMENT_GRID_BANNER_PERSONAL_TEXT');
		}
	}
	...
	private static function calculateStatus(Item\Member $member, Item\Document $document, Item\MemberCollection $memberCollection): array
	{
		$stageInfo = Ui\Member\Stage::createInstance($member, $document, $memberCollection)->getInfo();
