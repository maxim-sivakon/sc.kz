...
<body <?=$data_count?> class="type1 type1_light">
...
<section>
  <div class="block">
  	...
  	<div class="height_box">
        ...
        <? } ?>
    	    ...
    	    <div class="counter">
              ...
              <div class="counter_item item1"><span>0</span><i><?=GetMessage("DAYS");?></i></div>
              <div class="counter_separator item1"></div>
              <div class="counter_item item2"><span>0</span><i><?=GetMessage("HOURS");?></i></div>
              <div class="counter_separator item2"></div>
              <div class="counter_item item3"><span>0</span><i><?=GetMessage("MINUTES");?></i></div>
              <div class="counter_separator item3"></div>
              <div class="counter_item item4"><span>0</span><i><?=GetMessage("SECONDS");?></i></div>
            </div>
    	    </div>
      ...
      <? if( $phone || $phone2 || $email || $vk || $fb || $insta || $tele){ ?>
    	  <div class="side">
    	  	...
    	  	<div class="pseudo_table">
    	  	  <div class="pseudo_td height_item">
    	  	    ...
    	  	    <? if($phone || $phone2){ ?>
                <ul class="phones">
                  <?php include("include/phone_svg.php");?>
                  <? if($phone){ ?>
                    <li><a href="tel:<?=$phone?>" class="phone"><?=$phone?></a></li>
                  <? } ?>
                   <? if($phone2){ ?>
                    <li><a href="tel:<?=$phone2?>" class="phone"><?=$phone2?></a></li>
                  <? } ?>
                </ul>
              <? } ?>
          ...
          <? if($email){ ?>
                <ul class="mails">
                  <?php include("include/mail_svg.php");?>
                  <li><a href="mailto:<?=$email?>" class="mail"><?=$email?></a></li>
                </ul>
              <? } ?>
